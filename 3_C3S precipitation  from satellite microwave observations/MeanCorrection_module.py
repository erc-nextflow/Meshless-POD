...
def subtract_mean_vel2D(PTV, StartingGrid,UmEPTV):
    # Define the function that will be executed in parallel
    
    FinalGrid = np.transpose(np.array([PTV['X'].flatten(),PTV['Y'].flatten()]))
    meanUEPTV_PTV_linear = griddata(StartingGrid, UmEPTV.flatten(), FinalGrid, method='linear')
    meanUEPTV_PTV_nearest = griddata(StartingGrid, UmEPTV.flatten(), FinalGrid, method='nearest')
    meanUEPTV_PTV = np.where(np.isnan(meanUEPTV_PTV_linear), meanUEPTV_PTV_nearest,  meanUEPTV_PTV_linear)
    
    # meanVEPTV_PTV = griddata(StartingGrid, VmEPTV.flatten(), FinalGrid, method='nearest')
    u = PTV['T'].flatten() - meanUEPTV_PTV
...
def subtract_mean_velocities2D(PTV, StartingGrid, UmEPTV,NImg):
    # Define the main function that will call the parallel workers
    

    with mp.Pool(processes = 60) as pool:
        subtract_mean_vel_worker = partial(subtract_mean_vel2D, StartingGrid=StartingGrid,UmEPTV=UmEPTV)
        results = []
        for i in tqdm(NImg, desc='Processing snapshots', unit='snapshot'):
...
def MeanCorrection (PTV,NImg,EPTV_stat,flag):
    ...
    if flag == '3D':
        ...
        UmEPTV = EPTV_stat['TmEPTV']
        # VmEPTV = EPTV_stat['VmEPTV']
        X = EPTV_stat['X']
        Y = EPTV_stat['Y']
        StartingGrid = np.transpose(np.array([X.flatten(),Y.flatten()]))
        PTV = subtract_mean_velocities2D(PTV, StartingGrid, UmEPTV,NImg)
    else:
